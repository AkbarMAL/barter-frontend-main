...
export default function Dashboard() {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    debounceTimer.current = setTimeout(async () => {
      try {
        const res = await fetch(`${PRODUCT_API_BASE}/products?search=${encodeURIComponent(value.trim())}`);
        const json = await res.json();
        if (json.success) {
  ...
  const ProductCard = ({ p, showPromoBadge = false }: { p: any; showPromoBadge?: boolean }) => (
    <Link
      key={p.id}
      href={`/product/${p.id}`}
        ...
        {isSearching ? (
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={clearSearch} className="text-sm font-medium text-blue-600 hover:underline">
                Kembali ke Beranda
              </button>
            </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {promotedProducts.map((p) => (
                  <ProductCard key={p.id} p={p} showPromoBadge={true} />
                ))}
              </div>
            </section>
