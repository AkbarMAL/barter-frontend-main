...
import { saveAuthData, generateOAuthState } from "@/services/authentication";
...
export default function LoginPage() {
  ...
  const handleSocialLogin = async (provider: "google" | "facebook") => {
    ...
    if (!popup) {
      alert("Izinkan popup browser untuk login dengan Google/Facebook.");
      sessionStorage.removeItem("oauth_state");
      return;
    }

    popup.document.write(
      "<div style='font-family: sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; color: #666;'>Membuka halaman login Google/Facebook... silakan tunggu.</div>",
    );
    ...
    try {
      // Fetch endpoint untuk dapatkan redirect_url dari backend
      const apiBase = api.defaults.baseURL || "http://127.0.0.1:8000/api/v1";
      const res = await fetch(
        `${apiBase}/login/${provider}?redirect_uri=${encodeURIComponent(
          redirectUri,
        )}&state=${state}`,
      );
      ...
      const googleAuthUrl = data?.redirect_url || data?.data?.redirect_url;

      if (googleAuthUrl) {
        // Redirect popup ke Google OAuth URL
        popup.location.href = googleAuthUrl;
      } else {
        popup.document.write(
          "<div style='font-family: sans-serif; padding: 20px; color: #d32f2f;'>Error: Tidak dapat mendapatkan URL login dari server. Silakan coba lagi.</div>",
        );
      }
    } catch (err: any) {
      console.error("Social login error:", err.message);
      popup.document.write(
        `<div style='font-family: sans-serif; padding: 20px; color: #d32f2f;'>Error: ${err.message}</div>`,
      );
    }
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== "social-login-success") {
        return;
      }

      const { token, user, refresh_token } = data;
      if (token) {
        saveAuthData(token, user || {}, refresh_token);
        router.push(redirectUrl);
      }
